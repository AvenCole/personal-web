...
<template>
  <div class="relative flex flex-wrap justify-center items-center gap-[1em]" ref="containerRef">
    <span v-for="(word, index) in words" :key="props.index ? props.index[index] : index"
      :ref="el => setWordRef(el as HTMLSpanElement, index)"
      class="relative font-black text-7xl transition-[filter,color] duration-300 ease-in-out cursor-pointer" :style="{
        filter:
    ...
    <motion.div class="top-0 left-0 box-content absolute border-none pointer-events-none" :animate="{
      ...
      <span
        class="-top-2.5 -left-2.5 absolute filter-[drop-shadow(0_0_4px_var(--border-color,#fff))] border-[3px] border-(--border-color,#fff) border-r-0 border-b-0 rounded-[3px] w-4 h-4 transition-none"></span>

      <span
        class="-top-2.5 -right-2.5 absolute filter-[drop-shadow(0_0_4px_var(--border-color,#fff))] border-[3px] border-(--border-color,#fff) border-b-0 border-l-0 rounded-[3px] w-4 h-4 transition-none"></span>

      <span
        class="-bottom-2.5 -left-2.5 absolute filter-[drop-shadow(0_0_4px_var(--border-color,#fff))] border-[3px] border-(--border-color,#fff) border-t-0 border-r-0 rounded-[3px] w-4 h-4 transition-none"></span>

      <span
        class="-right-2.5 -bottom-2.5 absolute filter-[drop-shadow(0_0_4px_var(--border-color,#fff))] border-[3px] border-(--border-color,#fff) border-t-0 border-l-0 rounded-[3px] w-4 h-4 transition-none"></span>
    </motion.div>
  </div>
</template>
